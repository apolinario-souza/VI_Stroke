...
# -*- coding: utf-8 -*-
"""
Jogo de Coleta de Maçãs - Versão Final Corrigida com Múltiplas Frutas
"""
...
basket_img = load_image('img/personagem.png', alpha=True)

# Carrega todas as frutas possíveis
fruit_images = []
for i in range(1, 5):
    fruit_path = f'img/fruta{i}.png'
    fruit_img = load_image(fruit_path, alpha=True)
...
for i in range(len(fruit_images)):
    aspect_ratio = fruit_images[i].shape[1] / fruit_images[i].shape[0]
    apple_width = int(apple_size * aspect_ratio)
    fruit_images[i] = cv2.resize(fruit_images[i], (apple_width, apple_size))
...
class Apple:
    ...
    def draw(self, frame):
        if self.img is not None and self.img.shape[2] == 4:
            ...
            rotated_apple = cv2.warpAffine(self.img, rot_mat, (apple_width, apple_size))
...
def spawn_apple():
    global spawn_order
    fixed_x_positions = [
        track_start + track_width // 4,
        track_start + 3 * track_width // 4,
        track_start + track_width // 2
    ]
    if posicao_variavel == 0:
        x = fixed_x_positions[spawn_order]
        spawn_order = (spawn_order + 1) % 3
    else:
        x = random.choice(fixed_x_positions)
    
    fruit_img = random.choice(fruit_images)
    apples.append(Apple(x, fruit_img))
